const CHAMPION_FACTIONS_URL = "https://universe-meeps.leagueoflegends.com/v1/en_us/champion-browse/index.json"
...
function setFaction(factions: Record<string, string>, key: string, faction: string) {
...
function buildFactionLookup(champions: UniverseChampion[]): Record<string, string> {
...
export function getChampionFaction(
  ...
  return factions[normalizeChampionKey(champion.id)] ?? factions[normalizeChampionKey(champion.name)]
}

export async function fetchChampionFactions(): Promise<Record<string, string>> {
  if (!championFactionsPromise) {
    championFactionsPromise = fetchJson<UniverseChampionBrowseResponse>(CHAMPION_FACTIONS_URL)
